...
if template is None or template == '':
    template = ('first_name', 'last_name', 'email', 'street_address_1', 'street_address_2', 'town_village_city', 'province_state', 'postal_code', 'country', 'total_donations')
else:
    template = template.split(',')
condition = tow.form.getfirst('condition')
...
try:
 ...
 donation.total_donation
    from tow_person as p
       ...
       natural join (select person_id, sum(donation_amount) as total_donation from tow_donation where donation_date >= %(d_from)s and donation_date <= %(d_to)s and (donation_status = 'Unknown' or donation_status = 'Complete') group by person_id) as donation""", { 'd_from': d_from, 'd_to': d_to })
    for resp in tow.fetchrows():
        ...
        person['total_donation'] = tow.display_cash(resp[28])
